...
namespace HTTP
{
    ...
    TcpServer::TcpServer(std::string endpoint, int port) : m_endpoint(endpoint), m_port(port), m_socket(),
    ...
    std::string TcpServer::buildResponse()
    {
        ...
        Files::FileInfo htmlFile = files.RequestIndexFile();

        // set headers + contents
        ss << "HTTP/1.1 200 OK \nContent-Type: text/html\nContent-Length: " << htmlFile.contents.size() << "\n\n"
           << htmlFile.contents;
        return ss.str();
    }
